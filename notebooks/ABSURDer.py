...
class ABSURDer:
    ...
    def __init__(self, rex, rmd, eex = '', out = 'results', thetas=np.array([100,1000,10000]), idx=[],
                 methyl_list=None, verbose=True, ignore_last=None):
        ...
        self.ix2_nomderr = []                                        # initial chi2 before optimization without MD error
...
#        self._shift()
        for r in range(self.r):
            self.ix2_nomderr.append( self.chi2r_nomderr(r, self.w0) )
        self.ix2_nomderr.append( self.chi2r_nomderr(-1, self.w0) )
        ...
        if verbose:
            ...
            print(f"# Overall chi square (no MD error): {self.ix2_nomderr[-1]:.2f}")
...
#        chi2 = self._chi2_nomderr(r, w)                              # compute chi2 without MD errors
    ...
    #----------------------------------------------------------------------------------------------------------------
    #
    # def methyl_specific_rates(self, methyl_type):
    #
    #     self.methyl_specific = {'CBHB': [], 'CDHD': [], 'CDHD_I': [], 'CDHD_L': [], 'CG2HG2': [], 'CG2HG2_I': [],
    #                        'CG2HG2_V': [], 'CG2HG2_T': [], 'CD2HD2': [], 'CD1HD1': [], 'CEHE': [], 'CG1HG1': []}
    #
    #     for i, m in enumerate(self.mnl):
    #         tmp = m.split('-')[-1]
    #         if tmp in list(self.methyl_specific.keys()):
    #             self.methyl_specific[tmp].append(i)
    #         if 'CDHD' in m and 'ILE' in m:
    #             self.methyl_specific['CDHD_I'].append(i)
    #         elif 'CD1HD1' in m:
    #             self.methyl_specific['CDHD_L'].append(i)
    #             self.methyl_specific['CDHD'].append(i)
    #         elif 'CG2HG2' in m and 'ILE' in m:
    #             self.methyl_specific['CG2HG2_I'].append(i)
    #         elif 'CG2HG2' in m and 'VAL' in m:
    #             self.methyl_specific['CG2HG2_V'].append(i)
    #         elif 'CG2HG2' in m and 'THR' in m:
    #             self.methyl_specific['CG2HG2_V'].append(i)
    #
    #     self.rex_copy = np.copy(rex)
    #     self.eex_copy = np.copy(eex)
    #     self.rav_copy = np.copy(rav)
    ...
    def residue_specific_chi2( self, opt_theta = None ):

        res    = { 'ALA':[], 'ILE':[], 'LEU':[], 'MET':[], 'VAL':[], 'THR':[] }
        chi2   = { 'ALA':[], 'ILE':[], 'LEU':[], 'MET':[], 'VAL':[], 'THR':[] }
        chi2rw = { 'ALA':[], 'ILE':[], 'LEU':[], 'MET':[], 'VAL':[], 'THR':[] }

        for n, m in enumerate( self.mnl ):
            if 'ALA' in m:
                res['ALA'].append(n)
            elif 'ILE' in m:
                res['ILE'].append(n)
        ...
        for n, res in enumerate(res.keys()):

            tmp = []
            for r in range( rw.r ):
                diff = 0
                for m in res:
                    diff += (self.rav[r][m] - self.rex[r][m]) ** 2 / (self.eex[r][m] ** 2 + self.emd[r][m] ** 2) / len(res)
                tmp.append(diff)
            chi2[res].append(tmp)

            if opt_theta != None:
                tmp = []
                rrw = np.average(rw.rmd, weights = self.res[opt_theta], axis=-1)
                for r in range( rw.r ):
                    diff = 0
                    for m in res:
                        diff += (rrw[r][m] - self.rex[r][m]) ** 2 / (self.eex[r][m] ** 2 + self.emd[r][m] ** 2) / len(res)
                    tmp.append(diff)
                chi2rw[res].append(tmp)
