...
def parse():
    ...
    parser.add_argument( '--nopbc', type = str, required = False,
                        help = 'Path to the xtc file processed with -center, -pbc mol and -ur compact . If specified, this processing step will be skipped.' )
    parser.add_argument( '--rot_trans', type = str, required = False,
                        help = 'Path to the xtc file processed with -center, -pbc mol, -ur compact and -fit rot+trans . If specified, this processing step will be skipped.' )

    args = parser.parse_args()
    
    return args.xtc, args.tpr, args.gmx, args.out, args.b, args.e, args.lblocks_bb, args.lblocks_m, args.nopbc, args.rot_trans
    

########## User input ##########

xtc, tpr, gmx, out, b_frame, e_frame, l_blocks_bb, l_blocks_methyl, nopbc, rot_trans = parse()

print( f'--xtc {xtc} --tpr {tpr} --gmx {gmx} --out {out} --b {b_frame} --e {e_frame} --lblocks_bb {l_blocks_bb} -lblocks_m {l_blocks_methyl}' )
...
""" Prepare trajectory (BB): remove solvent & pbs & center """
if nopbc == None:
...
""" Prepare trajectory (Methyls): remove overall tumbling """
...
if os.path.isfile(initial):
    ...
    process  = f'{gmx} trjconv -s {tpr} -f {traj_nopbc} -o {initial} -b {b_frame} -e {b_frame}'
    print(process)
...
if os.path.exists( f'{out}/tcf_bb/{traj_nodir_bb}.pbz2'  ):
    for f in glob.glob(f'{out}/tcf_bb/*_{traj_nodir_bb}.xvg'):
        os.remove( f )
    print( "   # BB .xvg files deleted." )
                        
if os.path.exists( f'{out}/tcf_methyl/{traj_nodir_met}.pbz2'  ):
    for f in glob.glob(f'{out}/tcf_methyl/*_{traj_nodir_met}.xvg'):
        os.remove( f )
    print( "   # Methyl .xvg files deleted." )
